...
def _powder_brand_ids(db: Session) -> set[int]:
    ids: set[int] = set()
    settings = db.get(BookSettings, 1)
    if settings and settings.powder_brand_id:
        ids.add(settings.powder_brand_id)
    for brand_id in db.scalars(select(Brand.id).where(func.lower(Brand.name) == "powder")).all():
        ids.add(brand_id)
    return ids

def _validate_no_powder_output_lines(db: Session, output_lines: list[dict]) -> None:
    powder_brand_ids = _powder_brand_ids(db)
...
def _validate_powder_brand_id(db: Session, brand_id: int) -> None:
    if brand_id in _powder_brand_ids(db):
        return
    brand = db.get(Brand, brand_id)
    if brand and brand.name.strip().lower() == "powder":
        return
...
def _resolve_powder_product_id(db: Session) -> int:
    settings = db.get(BookSettings, 1)
    if settings and settings.powder_product_id:
        return settings.powder_product_id
    product_id = db.scalar(select(Product.id).where(func.lower(Product.product_name) == "powder").limit(1))
    if not product_id:
        raise ValueError("Create a product master named Powder (or set powder product in Book settings).")
    return product_id

def _resolve_powder_destination(db: Session) -> tuple[int, int, int, int, BagType]:
    settings = db.get(BookSettings, 1)
...
def _resolve_powder_for_batch(
    ...
    *,
    ...
    if powder_line:
        ...
        _validate_powder_brand_id(db, brand_id)
    ...
    if powder_kg_legacy > 0:
        _product_id, brand_id, location_id, bag_type_id, bt = _resolve_powder_destination(db)
        if bt.is_loose:
            bag_count = 0
...
def _allocate_powder_to_owners(
    ...
    if powder_kg <= 0:
        return
    product_id = _resolve_powder_product_id(db)
    brand_id = powder_line["brand_id"]
    location_id = powder_line["location_id"]
    bag_type_id = powder_line["bag_type_id"]
    bt = _processing._get_bag_type(db, bag_type_id)
