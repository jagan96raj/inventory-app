...
def list_users(db: Session, *, company_id: int) -> list[User]:
    return list(
        db.scalars(
            select(User).where(User.company_id == company_id).order_by(User.email.asc())
        ).all()
    )


def _assert_same_company(actor: User, user: User) -> None:
    if int(company_id_for_user(user)) != int(company_id_for_user(actor)):
    ...
    if user.role == UserRole.owner:
        owner_count = db.scalar(
            ...
            .where(
                ...
                User.company_id == company_id_for_user(user),
            )
        )
...
def set_user_active(db: Session, user_id: int, *, actor: User, is_active: bool) -> User:
    ...
    if not user:
        raise ValueError("User not found")
    _assert_same_company(actor, user)
    if not is_active:
...
def delete_user(
    ...
    *,
    ...
    act_id = actor.id if actor is not None else actor_id
    if act_id is None:
