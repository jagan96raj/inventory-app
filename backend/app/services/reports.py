...
def _bill_filters(year: int, month: int, bill_type: BillType):
...
def _month_bill_totals(db: Session, year: int, month: int, bill_type: BillType) -> dict:

    """Bill-date totals for one bill type (v11.1 primary metrics)."""

    filters = _bill_filters(year, month, bill_type)
...
def _month_sales_legacy_totals(db: Session, year: int, month: int) -> dict:
...
def get_business_summary(db: Session, year: int, month: int) -> dict:

    sales = _month_bill_totals(db, year, month, BillType.sales)

    purchase = _month_bill_totals(db, year, month, BillType.purchase)
...
def get_business_compare(db: Session, year: int, month: int) -> dict:

    cur_sales = _month_bill_totals(db, year, month, BillType.sales)

    cur_purchase = _month_bill_totals(db, year, month, BillType.purchase)

    py, pm = prev_year_month(year, month)

    prev_sales = _month_bill_totals(db, py, pm, BillType.sales)

    prev_purchase = _month_bill_totals(db, py, pm, BillType.purchase)
...
def get_dashboard_bundle(
    ...
    return {
        ...
        "daily": get_daily_bill_amounts(db, year, month),

        "by_product": get_bills_by_product(db, year, month, bill_type, group_by),

        "by_customer": get_bills_by_customer(db, year, month, bill_type, limit=10),

        "by_location": get_bills_by_location(db, year, month, bill_type),
...
def get_sales_summary(db: Session, year: int, month: int) -> dict:

    current = _month_sales_legacy_totals(db, year, month)

    py, pm = prev_year_month(year, month)

    prev = _month_sales_legacy_totals(db, py, pm)
...
def get_sales_compare(db: Session, year: int, month: int) -> dict:

    current = _month_sales_legacy_totals(db, year, month)

    py, pm = prev_year_month(year, month)

    previous = _month_sales_legacy_totals(db, py, pm)
...
def get_bills_by_product(
    ...
    db: Session, year: int, month: int, bill_type: BillType, group_by: str
...
def get_sales_by_product(db: Session, year: int, month: int, group_by: str) -> dict:

    return get_bills_by_product(db, year, month, BillType.sales, group_by)
...
def get_bills_by_customer(
    ...
    db: Session, year: int, month: int, bill_type: BillType, limit: int = 10
...
def get_sales_by_customer(db: Session, year: int, month: int, limit: int = 10) -> dict:

    return get_bills_by_customer(db, year, month, BillType.sales, limit)





def get_bills_by_location(db: Session, year: int, month: int, bill_type: BillType) -> dict:
...
def get_sales_by_location(db: Session, year: int, month: int) -> dict:

    return get_bills_by_location(db, year, month, BillType.sales)





def get_daily_bill_amounts(db: Session, year: int, month: int) -> dict:
...
def get_sales_daily(db: Session, year: int, month: int) -> dict:
...
def _status_breakdown(db: Session, year: int, month: int, field) -> dict:
...
def get_sales_payment_breakdown(db: Session, year: int, month: int) -> dict:

    raw = _status_breakdown(db, year, month, Bill.payment_status)
...
def get_sales_delivery_breakdown(db: Session, year: int, month: int) -> dict:

    raw = _status_breakdown(db, year, month, Bill.order_delivery_status)
...
def get_bills_export_csv(
    ...
    db: Session, year: int, month: int, bill_type: BillType, group_by: str = "product_brand"

) -> str:

    data = get_bills_by_product(db, year, month, bill_type, group_by)
...
def get_sales_export_csv(db: Session, year: int, month: int, group_by: str = "product_brand") -> str:

    return get_bills_export_csv(db, year, month, BillType.sales, group_by)
