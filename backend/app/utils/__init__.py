...
def validate_bags_loose(
    ...
    *,
    ...
    """Validate bags/loose for a transaction line.

    By default quantity must be > 0. Bill lines may pass allow_zero=True so an
    ordered line can be reduced to zero bags/kg (e.g. drop a product on edit).
    """
    if bag_type.is_loose:
        if bag_count != 0:
            raise ValueError("Loose bag type cannot have bag count")
        if loose_kg < 0:
            raise ValueError("Loose quantity cannot be negative")
        if not allow_zero and loose_kg <= 0:
            raise ValueError("Loose quantity must be greater than zero")
    else:
        if bag_count < 0:
            raise ValueError("Bag count cannot be negative")
        if not allow_zero and bag_count <= 0:
...
def delivery_status_from_qty(ordered: Decimal, net: Decimal) -> str:
    # Zero-ordered lines are complete (nothing left to deliver/receive).
    if ordered <= 0:
        return "delivered"
    if net <= 0:
