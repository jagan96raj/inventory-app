...
**Spec range:** v5 (bills / payments / edit) through **v17.3.29** (Nginx security headers + Turnstile auth bot protection; v17.3.28 IDOR/rate limits/gitleaks); inventory **v14.2.1**; money accounts **v17.2.0–v17.2.4**; backend **v12.21** + **v12.22** amendments
...
**Problem:** The v17.3.24 hint mixed “not delivered” with a reserved formula that excluded the oldest bill and could go negative. Operators need Available = godown qty for **this bag type**, and Reserved = what other open sales bills of **that same SKU** still need.

**Solution:**
- Scope is product + brand + bag type + location + owned vs job-work (customer). Do **not** mix other bag types.
- **Available** = physical on hand of that SKU (drops only on Deliver). Packed bag type → bags (e.g. `Available 50 bags`). Loose → kg.
- **Reserved** = remaining (ordered − delivered) on **other** open (non-void) sales bills of that SKU only — not the current bill. Packed → bags; loose → kg.
- If reserved is 0, show **Available only**.
- Examples: 50 bags on hand, bill A 100 → Available 50 bags only. Deliver 20 of A → on hand 30 bags, A remaining 80. Bill B 100 → Available 30 bags, Reserved 80 bags. Bill C 100 → Available 30 bags, Reserved 180 bags.
- `GET /api/bills/sales-stock-hints` adds `on_hand_bags` and per-bill `remaining_bags`. Display-only; no Alembic.

**Unchanged:** Create still does not write inventory; deliver cannot exceed physical on_hand; money math; olive brand; no desktop-shell.
...
**Problem:** Sales create only listed SKUs with stock at the bill location, so 0-qty / missing inventory rows could not be billed. Over-on-hand create was blocked even though deliver is the only stock movement. Operators need a display of physical on-hand vs later bills vs total not delivered — without reserving rows or changing money math.

**Solution:**
- **Create dropdowns:** Product / brand / bag type use the same master search as purchase. Missing inventory row = on_hand 0; line still allowed.
- **Hint (same SKU + location + owner):** Available = on_hand (physical; billing never changes this). Not delivered = SUM(ordered − delivered) on active non-void sales bills for that SKU, including current form lines; when editing, exclude the current bill’s saved lines and replace with the form. Oldest active undelivered sales bill is **not** counted in reserved. If only one open bill: reserved = 0. If 2+: later_qty = remaining on the other open bills except that oldest; reserved = on_hand − later_qty (may be negative → warning tone).
- **Examples:** 50 on hand + one bill 100 → available 50, reserved 0, not delivered 100. Second 100 → reserved −50, not delivered 200. Third → reserved −150. 200 on hand + one 100 → reserved 0; + second 100 → reserved 100.
- **Submit:** Do **not** block create when qty > on_hand (warning only).
- **Fulfillment (first Deliver wins):** Open dialogs may show the same on_hand. After one deliver succeeds, other open dialogs refetch stock. Submit over remaining on_hand is rejected. No older-bill FIFO on the server.
- **Read API:** `GET /api/bills/sales-stock-hints?location_id=&exclude_bill_id=` returns on_hand + open bill remainings. No reserved column, no negative stock in DB, no Alembic.

**Unchanged:** Bill money math; payments/balances; deliver is the only stock movement; olive brand; no desktop-shell.
...
**Problem:** Owner needs a light company notepad for reminders and mill tips, optionally shared with specific staff roles — not a full chat or docs product.

**Solution:**
- Route `/notes` under Overview; permission `notes_view` for all assigned roles.
- Tables: `company_notes` (title nullable, body required, `note_date`, author FKs) + `company_note_role_access(note_id, role)` for viewer roles `writer` / `stock_manager` / `factory_manager`. Empty access rows = **owner-only**.
- API: `GET/POST /api/notes`, `PATCH/DELETE /api/notes/{id}`. List filtered by role (owner sees all). `PATCH` may set `viewer_roles`. Non-owner mutate → **403**; note not visible → **404**.
- UI: soft cream paper cards (slight tilt + float), badge Owner only / Shared with…; owner overflow → Edit, Delete (`ConfirmDialog`), Who can see… (checkboxes + Save). Non-owners read-only. FAB Add for owner on phone.
- Migration `062_spec_v17322_company_notes`. No desktop-shell change.

**Unchanged:** Existing RBAC elsewhere; no note restore/history.
...
**Problem:** After Phases 1–6, leftover phone pain remained in account masters, audit/login histories, shared dialog footers, and long forms whose primary CTA scrolled off-screen.

**Solution (Phase 7 only — layout/CSS; no business-logic or new features):**
- **Shared chrome:** `Modal` footer stacks + full-width buttons on xs; `CardFooter`/`PageHeader`/`EmptyState` mobile-friendly defaults.
- **Money accounts + expense categories:** Cards below `lg`; desktop tables from `lg`; modal footers use Modal default stack.
- **Audit log + login history:** Cards below `lg`; filter grids `min-w-0`.
- **Sticky actions:** Bill form, job-work create, bag-change / transfer / disposal, cash-book entry footers stick on scroll with phone-friendly button widths.
- **Dialogs:** Inventory add/edit stock + processing open-job action rows stack on xs.
- **Out of scope:** New product features; intentional redesign beyond leftover mobile polish.

**Unchanged:** Aurora design language; APIs and business rules.
